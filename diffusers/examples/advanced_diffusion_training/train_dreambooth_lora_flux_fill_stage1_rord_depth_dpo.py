...
class SimpleMLPClassifier(nn.Module):
    def __init__(self, input_dim=1024, hidden_dim=256, num_classes=1):
        ...
        self.classifier_2 = nn.Linear(68, num_classes)
...
def make_edge_attached_mask(mask_path, mode="irregular",
                            bbox_min_size=60, bbox_max_size=200, margin=20,
                            min_times=1, max_times=2,
                            max_angle=4, max_len=120, max_width=40):
    # === 1. Load original mask and extract boundary ===
    ...
    boundary = dilated - mask  # 1-pixel outer boundary

    ys, xs = np.where(boundary > 0)
    if len(xs) == 0:
        raise ValueError("No boundary pixels found.")
    ...
    for i in range(times):
        # === Sample starting point from boundary ===
        ...
        if mode == "rectangle":
            ...
            top = np.clip(start_y - box_height // 2, margin, height - margin - box_height)
            left = np.clip(start_x - box_width // 2, margin, width - margin - box_width)

            edge_mask[top:top + box_height, left:left + box_width] = 1.0
            ...
            for j in range(1 + np.random.randint(5)):
                ...
                if choice == 0:
                    cv2.line(edge_mask, (start_x, start_y), (end_x, end_y), 1.0, brush_w)
                elif choice == 1:
                    cv2.circle(edge_mask, (start_x, start_y), radius=brush_w, color=1.0, thickness=-1)
                elif choice == 2:
                    radius = brush_w // 2
                    edge_mask[max(0, start_y - radius):min(height, start_y + radius),
                              max(0, start_x - radius):min(width, start_x + radius)] = 1.0

                start_x, start_y = end_x, end_y
        else:
            raise ValueError("Unknown mode: choose 'rectangle' or 'irregular'")

    # === Ensure no overlap with original mask ===
    edge_mask[mask == 1] = 0

    return (edge_mask * 255).astype(np.uint8)

def make_random_rectangle_mask(
    height, width, margin=20, bbox_min_size=60, bbox_max_size=200, min_times=0, max_times=3
):
...
def make_random_irregular_mask(height, width, max_angle=4, max_len=120, max_width=40, min_times=0, max_times=10):
    mask = np.zeros((height, width), np.float32)
...
def prepare_mask_latents(
    ...
    unpacked_masked_image_latents = masked_image_latents
    ...
    return mask, masked_image_latents, unpacked_masked_image_latents
...
def log_validation(
    ...
    images = []
    with autocast_ctx:
        validation_dirs = ["example_data/I-210618_I01001_W01_I-210618_I01001_W01_F0153_img.jpg"]

        for validation_dir in validation_dirs:
            image = Image.open(validation_dir) # place_hold
            mask_path = validation_dir.replace("_img.jpg","_mask.png")
            mask = Image.open(mask_path).convert("L")
            mask = ImageOps.invert(mask)    # inverse rord_mask
            depth = Image.open(validation_dir.replace("_img.jpg", 
                                                      "_depth_img.png"))
            image = pipeline(
...
class RORD_mask(Dataset):
    ...
    def __getitem__(self, index):
        ...
        example = {}
        true_index = index % self.num_instance_images
        image_path = self.image_paths[true_index]
        mask_path = self.mask_image[true_index]

        image = Image.open(image_path).convert("RGB")
        gt = Image.open(image_path.replace("/img/", "/gt/")).convert("RGB")

        image_depth = Image.open(image_path.replace("/img/", "/img_depth/")).convert("RGB")
        gt_depth = Image.open(image_path.replace("/img/", "/gt_depth/")).convert("RGB")
        mask = Image.open(mask_path).convert("L")
        ...
        self.mask_transform = transforms.Compose([
            transforms.Resize([width, height], interpolation=transforms.InterpolationMode.NEAREST),
            transforms.ToTensor(),
        ])

        image = self.image_transforms(image)
        gt = self.image_transforms(gt)
        image_depth = self.image_transforms(image_depth)
        gt_depth = self.image_transforms(gt_depth)

        mask_rord = Image.open(mask_path).convert("L")
        example = {}
        example["removal_loss_gd_mask"] = torch.tensor([0])
        # Convert back to PIL image and apply transforms
        mask_rord = self.mask_transform(mask_rord)  
        mask_rord[mask_rord < 0.5] = 0
        mask_rord[mask_rord >= 0.5] = 1
        mask = 1 - mask_rord

        if random.random() < 0.5:
            masked_depth_rmoval = gt_depth * (1 - mask)
        else:
            masked_depth_rmoval = image_depth * (1 - mask)

        example["removal_instance_images"] = gt_depth
        example["removal_masked_image"] = masked_depth_rmoval
        example["removal_mask"] = mask
        example["removal_instance_prompt"] = self.instance_prompt
        example["removal_cropped_image"] = gt
        example["removal_instance_images_neg"] = image_depth
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    pixel_values_neg = [example["removal_instance_images_neg"] for example in examples]
    pixel_values = [example["removal_instance_images"] for example in examples]

    prompts = [example["removal_instance_prompt"] for example in examples]
    masked_images = [example["removal_masked_image"] for example in examples]
    masks = [example["removal_mask"] for example in examples]
    masks_gt_guidance = [example["removal_loss_gd_mask"] for example in examples]
    crop_pixel_values = [example["removal_cropped_image"] for example in examples]
    ...
    pixel_values_neg = pixel_values_neg.to(memory_format=torch.contiguous_format).float()

    crop_pixel_values = torch.stack(crop_pixel_values)
    crop_pixel_values = crop_pixel_values.to(memory_format=torch.contiguous_format).float()
    ...
    masks_gt_guidance = torch.stack(masks_gt_guidance)
    masks_gt_guidance = masks_gt_guidance.to(memory_format=torch.contiguous_format).float()

    batch = {"pixel_values": pixel_values, "prompts": prompts, "masks": masks, "crop_pixel_values": crop_pixel_values,
             "masked_images": masked_images, "masks_gt_guidance": masks_gt_guidance, "pixel_values_neg": pixel_values_neg, }
    return batch
...
def main(args):
    ...
    def load_transformer_only_hook(models, input_dir):
        transformer_ = None
        ...
        while len(models) > 0:
            ...
            if isinstance(model, type(unwrap_model(transformer))):
                ...
                logger.warning(f"Unexpected model encountered: {model.__class__}")
    ...
    train_dataset= RORD_mask(
        ...
        csv_file="train_images_and_rord_masks.csv",
        partition="train"
    )
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers(tracker_name, config=vars(args), init_kwargs={"wandb": {"entity": "buxiangzhiren", "id": "Rank_64_joint_filter_stage1_points_depth_rose_res1024_dpo_flow_0.01"}})
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer):
                ...
                pixel_values_neg = batch["pixel_values_neg"].to(dtype=vae.dtype)
                model_input_neg = vae.encode(pixel_values_neg).latent_dist.sample()
                model_input_neg = (model_input_neg - vae_config_shift_factor) * vae_config_scaling_factor
                model_input_neg = model_input_neg.to(dtype=weight_dtype)
                ...
                sigmas = get_sigmas(timesteps, n_dim=model_input.ndim, dtype=model_input.dtype)
                noisy_model_input = (1.0 - sigmas) * model_input + sigmas * noise
                noisy_model_input_neg = (1.0 - sigmas) * model_input_neg + sigmas * noise
                ...
                packed_noisy_model_input_neg = FluxFillPipeline._pack_latents(
                    noisy_model_input_neg,
                    batch_size=model_input.shape[0],
                    num_channels_latents=model_input.shape[1],
                    height=model_input.shape[2],
                    width=model_input.shape[3],
                ...
                if accelerator.unwrap_model(transformer).config.guidance_embeds:
                    ...
                    guidance = None
                masked_image = batch["masked_images"]
                mask_image = batch["masks"]
                masks_gt_guidance = batch["masks_gt_guidance"]
                # cropped_image = batch['crop_pixel_values']

                mask, masked_image_latents, unpacked_masked_image_latents = prepare_mask_latents(
                    vae_scale_factor=8,
                    vae=vae,
                    mask=mask_image,
                ...
                model_pred_neg = transformer(
                    hidden_states=torch.cat((packed_noisy_model_input_neg, masked_image_latents), dim=2),
                    # YiYi notes: divide it by 1000 for now because we scale it by 1000 in the transforme rmodel (we should not keep it but I want to keep the inputs same for the model for testing)
                    timestep=timesteps / 1000,
                    guidance=guidance,
                    pooled_projections=pooled_prompt_embeds,
                ...
                model_pred_neg = FluxFillPipeline._unpack_latents(
                    model_pred_neg,
                    height=model_input.shape[2] * vae_scale_factor,
                    width=model_input.shape[3] * vae_scale_factor,
                    vae_scale_factor=vae_scale_factor,
                ...
                target_neg = noise - model_input_neg
                def compute_flow(depth):

                    flow_x = depth[:, :, :, 1:] - depth[:, :, :, :-1]  # (B, 1, H, W-1)
                    flow_y = depth[:, :, 1:, :] - depth[:, :, :-1, :]  # (B, 1, H-1, W)
                    return flow_x, flow_y

                def flow_loss(pred, gt):
                    flow_pred_x, flow_pred_y = compute_flow(pred)
                    flow_gt_x, flow_gt_y = compute_flow(gt)

                    loss_x = torch.nn.functional.l1_loss(flow_pred_x, flow_gt_x, reduction='none')
                    loss_y = torch.nn.functional.l1_loss(flow_pred_y, flow_gt_y, reduction='none')
                ...
                loss_x_pos, loss_y_pos = flow_loss(model_pred, target)
                loss_x_neg, loss_y_neg = flow_loss(model_pred_neg, target_neg)

                loss_dpo_x = -torch.log(torch.sigmoid(loss_x_neg - loss_x_pos))
                loss_dpo_y = -torch.log(torch.sigmoid(loss_y_neg - loss_y_pos))

                loss_dpo = (loss_dpo_x.mean() + loss_dpo_y.mean()) / 2

                loss = loss.mean() + 0.01 * loss_dpo
            ...
            logs = {"loss": loss.detach().item() - 0.01 * loss_dpo.detach().item(), "lr": lr_scheduler.get_last_lr()[0], "loss_dpo": loss_dpo.detach().item(), }
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and (epoch % args.validation_epochs == 0 or global_step % args.checkpointing_steps == 0):
                # create pipeline
                if freeze_text_encoder:
                    text_encoder_one, text_encoder_two = load_text_encoders(text_encoder_cls_one, text_encoder_cls_two)
                pipeline = FluxFillPipeline.from_pretrained(
