...
def log_validation(
    ...
    with autocast_ctx:
        ...
        for validation_dir in validation_dirs:
            image = Image.open(validation_dir)
            mask = Image.new("L", image.size, 0)  # place_hold
            depth_path = validation_dir.replace("_img.jpg", "_depth_img.png")
            depth_image = Image.open(depth_path)
            depth = Image.open(depth_path.replace("_img", "_gt"))
            image = pipeline(
...
class RORD(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if random.random() < 0.9:     
            example["removal_instance_images"] = torch.cat([gt, gt_depth, image_depth], dim=-1)
            example["removal_masked_image"] = torch.cat([image, gt_depth, image_depth], dim=-1)
            example["removal_mask"] = torch.cat([torch.zeros_like(mask), torch.zeros_like(mask), torch.zeros_like(mask)], dim=-1)
            example["removal_instance_prompt"] = self.instance_prompt
            example["removal_cropped_image"] = gt
        else:
            example["removal_instance_images"] = torch.cat([image, image_depth, gt_depth], dim=-1)
            example["removal_masked_image"] = torch.cat([gt, image_depth, gt_depth], dim=-1)
            example["removal_mask"] = torch.cat([torch.zeros_like(mask), torch.zeros_like(mask), torch.zeros_like(mask)], dim=-1)
            example["removal_instance_prompt"] = self.instance_prompt
            example["removal_cropped_image"] = gt
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    batch = {"pixel_values": pixel_values, "prompts": prompts, "masks": masks, "crop_pixel_values": crop_pixel_values,
             "masked_images": masked_images, "masks_gt_guidance": masks_gt_guidance}
    return batch
...
def main(args):
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers(tracker_name, config=vars(args), init_kwargs={"wandb": {"entity": "xxxx", "id": "xxxxx"}})
    # Train!
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer):
                ...
                image_pred = model_pred[:, :, :, :third_w]
                cond1_pred = model_pred[:, :, :, third_w:2*third_w]
                cond2_pred = model_pred[:, :, :, 2*third_w:]

                image_target = target[:, :, :, :third_w]
                cond1_target = target[:, :, :, third_w:2*third_w]
                cond2_target = target[:, :, :, 2*third_w:]

                image_loss = ((weighting.float() * (image_pred - image_target) ** 2).reshape(B, -1)).mean(dim=1)
                cond1_loss = ((weighting.float() * (cond1_pred - cond1_target) ** 2).reshape(B, -1)).mean(dim=1)
                cond2_loss = ((weighting.float() * (cond2_pred - cond2_target) ** 2).reshape(B, -1)).mean(dim=1)

                pixel_loss = image_loss.mean()
                condition_loss = 0.5 * (cond1_loss + cond2_loss).mean()
                if args.with_prior_preservation:
            ...
            logs = {"loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0], "pixel_loss": pixel_loss.detach().item(), "condition_loss": condition_loss.detach().item()}
